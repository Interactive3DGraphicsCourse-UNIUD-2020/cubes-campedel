...
function generateTree(container, treeHeight = 5, treeWidth = 2, wireframe = false) {
	...
	// creazione del container per la singola geoemtria che combina tutti i cubi per il legno
	let combinedWoodGeometry = new THREE.Geometry();
	// creazione del container per la singola geoemtria che combina tutti i cubi per le foglie
	let combinedLeafsGeometry = new THREE.Geometry();
		...
		if (y == lastBaseCubeAdded) {
			...
			for (let z = -startingCoordinate; z <= startingCoordinate; z++)
				...
				for (let x = -startingCoordinate; x <= startingCoordinate; x++) {
					...
					// aggiornamento della matrice di posizione della mesh, dal momento che questa è stata cambiata rispetto a quella di default
					meshLeafs.updateMatrix();
					// aggiunta della mesh per il cubo appena creato all'interno della geometria che combina tutti i cubi
					combinedLeafsGeometry.merge(meshLeafs.geometry, meshLeafs.matrix);
				}
	...
	// creazione della mesh a partire dalla geoemtria che combina tutti i cubi
	let combinedLeafsMesh = new THREE.Mesh(combinedLeafsGeometry, materialLeaf);
	// comando che permette alla mesh di generare ombre
	combinedLeafsMesh.castShadow = true;
	// comando che permette alla mesh di ricevere ombre
	combinedLeafsMesh.receiveShadow = true;
	// aggiunta della mesh al container passato alla funzione
	container.add(combinedLeafsMesh);
}